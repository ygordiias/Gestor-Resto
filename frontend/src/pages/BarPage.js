...
import { Wine, Clock, Check, Timer, Bell } from 'lucide-react';
...
export default function BarPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        ...
        // Conta itens pendentes de bebida
        let newPendingCount = 0;
        newOrders.forEach(order => {
          order.items?.forEach(item => {
            if (item.type === 'drink' && item.status === 'pending') {
              newPendingCount++;
            }
          });
        });
        ...
        if (newPendingCount > prevPendingCountRef.current && prevPendingCountRef.current > 0) {
          ...
          toast.info('Novo pedido no bar!', { icon: <Bell className="h-4 w-4" /> });
        }
  ...
  const updateItemStatus = async (orderId, itemId, newStatus, tableNumber, productName) => {
    try {
      ...
      if (newStatus === 'ready') {
        socketService.emit('item_ready', { tableNumber, productName, type: 'drink' });
  ...
  const getBarItems = () => {
    ...
    orders.forEach(order => {
      order.items
        ?.filter(item => item.type === 'drink' && item.status !== 'delivered')
        .forEach(item => {
          items.push({
            ...item,
            orderId: order.id,
  ...
  const pendingItems = barItems.filter(i => i.status === 'pending');
